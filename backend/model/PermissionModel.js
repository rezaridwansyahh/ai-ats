...
class Permission {
  ...
  static async getByIdDetails(id) {
    const result = await db.query(`
      SELECT 
        perm.*,
        r.id AS role_id,
  ...
  static async checkPermissionMultipleRoles(roleIds, moduleName, menuName, functionality) {
  ...
  static async checkPermissionsRoleId(roleIds) {
