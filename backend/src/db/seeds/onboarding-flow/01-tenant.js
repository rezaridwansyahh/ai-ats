// Step 1/5 — tenant: company + role (with permissions) + user.
//
// Run with:
//   cd backend && node src/db/seeds/onboarding-flow/01-tenant.js
//
...
import { COMPANY, ROLE, USER } from './config.js';
...
async function run() {
  ...
  try {
    ...
    console.log(`+ Granted ${grantedCount} new permission(s) to role "${ROLE.name}"`);
    ...
    console.log('\nNext: node src/db/seeds/onboarding-flow/02-job.js');
  } catch (err) {
