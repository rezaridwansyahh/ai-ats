// Step 2/4 — normalizes Battery B's hardcoded question banks
// (frontend/src/components/assessment-b/data/{tk,epps,holland,papi}.js)
// into assessment_subtest + assessment_question rows.
//
// Run with:
//   cd backend && node src/db/seeds/assessment-questions/02-battery-b.js
//
// Requires: 01-battery-a.js's helpers pattern; migration 019 applied;
// master_assessment seeded (assessment_code = 'myralix_battery_b').
// Safe to re-run — same upsert-subtest / replace-questions approach as Battery A.

import '../../../config/env.js';
import getDb from '../../../config/postgres.js';

import { SUBS, KEYS, WPT_RAW_QS, DATQS } from '../../../../../frontend/src/components/assessment-b/data/tk.js';
import { ITEMS as EPPS_ITEMS, EPPS_ANSKEY } from '../../../../../frontend/src/components/assessment-b/data/epps.js';
import { HOL_QS } from '../../../../../frontend/src/components/assessment-b/data/holland.js';
import { PAPI_QS, SCORING_KEY as PAPI_SCORING_KEY } from '../../../../../frontend/src/components/assessment-b/data/papi.js';

const ASSESSMENT_CODE = 'myralix_battery_b';
...
// Shared shape across every TK sub-part: {s, o:{A..E}, a} for PV/KN/PA/KA,
// or {n,type,text,opts?,hint?,svgHtml?} + KEYS for GI.
...
async function run() {
  ...
  // ── Holland RIASEC (identical item bank to Battery A) ────────────────────
  ...
  console.log('Battery B question bank seeded successfully.');
  process.exit(0);
}

run().catch((err) => {
  console.error('Battery B seed failed:', err);
  process.exit(1);
});
