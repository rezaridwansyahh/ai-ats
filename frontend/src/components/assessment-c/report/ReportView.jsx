...
const NARR_IDS = ['narr-tk', 'narr-epps', 'narr-papi', 'narr-sjt', 'narr-konsol', 'narr-strength', 'narr-dev', 'narr-fit', 'narr-rec'];
    ...
    <div className="max-w-[1100px] mx-auto px-4 py-5 pb-20">
      ...
      >
        ...
        <div className="pt-5 space-y-4">
          ...
          {/* SECTION III — PAPI */}
          <SectionCard num="III" title="Preferensi Kerja — PAPI Standard" subtitle="10 Peran + 10 Kebutuhan · 90 Pasang" color="#0891B2" bg="#ECFEFF">
            {papi ? (
              <>
          ...
          {/* SECTION IV — SJT */}
          <SectionCard num="IV" title="Penilaian Situasional — SJT" subtitle="22 Skenario · 6 Kompetensi Kepemimpinan" color="#6366F1" bg="#EEF2FF">
            {sjt ? (
              <>
                <div className="flex items-start gap-5 flex-wrap mb-3.5">
                  <div className="min-w-[200px]">
                    <div className="text-[10px] font-bold tracking-wider uppercase text-indigo-600 mb-1">Profil Kepemimpinan</div>
                    <div className="font-serif text-4xl font-bold tracking-widest leading-none" style={{ color: sjtProfile?.color || '#6366F1' }}>
                      {sjt.profile || '—'}
                    </div>
                    <div className="text-sm font-semibold mt-1" style={{ color: sjtProfile?.color || '#6366F1' }}>
                      {sjtProfile?.name}
                    </div>
                    ...
                    <div className="flex gap-1.5 mt-2 flex-wrap">
                      ...
                      <span className="px-2.5 py-0.5 rounded-full text-[10.5px] font-bold bg-indigo-100 text-indigo-700">
                    ...
                    {COMP_ORDER.map((k) => {
                      ...
                      return (
                        <div key={k} className="flex items-center gap-2 mb-1.5">
                          <span className="w-9 font-extrabold text-[10px] uppercase" style={{ color: meta.color }}>{k}</span>
                          <div className="flex-1 bg-slate-100 rounded-full h-1.5 overflow-hidden">
                            <div className="h-1.5 rounded-full" style={{ width: `${pct}%`, background: meta.color }} />
                          </div>
                          <span className="text-xs font-semibold min-w-[40px] text-right" style={{ color: meta.color }}>
                            {raw}/{meta.maxScore}
                          </span>
                ...
                {sjtProfile && (
                  <div className="bg-indigo-50 border-l-4 border-indigo-500 rounded-r px-3.5 py-2.5 text-xs text-slate-600 leading-relaxed">
                    <strong>{sjtProfile.name}:</strong> {sjtProfile.desc}
                  </div>
                )}
              </>
            ) : (
            ...
            <AssessorNotes id="notes-sjt" state={state} setNotes={setNotes} placeholder="Catatan tentang penilaian situasional kepemimpinan..." />
          </SectionCard>

          {/* SECTION V — Summary */}
          <SectionCard num="V" title="Ringkasan & Rekomendasi Akhir" subtitle="Konsolidasi seluruh asesmen Battery C" color="#064E3B" bg="#EDF7F5" emphasized>
