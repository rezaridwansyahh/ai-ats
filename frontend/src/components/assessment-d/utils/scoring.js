...
// ──────── Battery-C-specific scorers ────────

// SJT-specific level label (more granular than getVerdict).
...
/**
 * Score all SJT scenarios → competency totals → profile assignment.
 * @param answers Array of 22 entries, each is the chosen option index (0..3) or null.
 * @param SJT_QS  data/sjt.js SJT_QS array (each item has {comp, opts:[{s}]}).
 * @param COMPS   data/sjt.js COMPS object (each has {maxScore}).
 * Returns { compScores, compPct, totalScore, overallPct, score10, verdict, profile }.
 */
export function scoreSJT(answers, SJT_QS, COMPS) {
  ...
  answers.forEach((idx, i) => {
    ...
    const item = SJT_QS[i];
    if (!item) return;
    const opt = item.opts[idx];
    if (!opt) return;
    const s = opt.s ?? 0;
    if (compScores[item.comp] != null) compScores[item.comp] += s;
    totalScore += s;
  });

  const compPct = {};
  for (const k of Object.keys(compScores)) {
    const max = COMPS[k]?.maxScore || 1;
    compPct[k] = Math.round((compScores[k] / max) * 100);
  }

  // Per mockup: total max = 66 across all six competencies.
  const totalMax = Object.values(COMPS).reduce((sum, c) => sum + (c.maxScore || 0), 0) || 66;
  ...
  // Profile assignment (mockup submitSJT lines 2190–2196).
  let profile;
  if (overallPct >= 78) {
    profile = 'BJ';
