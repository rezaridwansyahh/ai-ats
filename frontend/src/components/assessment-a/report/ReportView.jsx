...
const SECTION_BY_NARR = {
  'narr-tk': 'tk',
  'narr-bigfive': 'bigfive',
  'narr-disc': 'disc',
  'narr-holland': 'holland',
};
...
export default function ReportView({ profile, results, state, updateState, saveNow }) {
  ...
  const onGenerateSynthesis = () => ai.generateSynthesis({
    battery: BATTERY,
    allScores: { tk, bigfive, disc, holland: hol },
  ...
  return (
    <div className="max-w-[1100px] mx-auto px-4 py-5 pb-20">
      ...
      >
        <div className="flex items-center justify-between px-6 py-3.5 border-b border-white/10 flex-wrap gap-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="bg-white/15 border border-white/25 rounded-full px-3 py-1 text-[11px] font-bold inline-flex items-center gap-1">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-x-3.5 gap-y-1.5 max-w-[420px]">
              {[
                ...
                ['Tanggal Tes', profile?.date ? fmtDateID(new Date(profile.date)) : (results.holland?.date || results.disc?.date || results.bigfive?.date || results.tk?.date || fmtDateID())],
                ['Email', profile?.email || '—'],
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-lg px-4 py-2.5 text-xs text-amber-900 leading-relaxed mb-4">
        <strong className="inline-flex items-center gap-1"> <AlertTriangle className="w-3.5 h-3.5" /> Catatan:  </strong> 
        Laporan ini merupakan alat bantu profesional. Interpretasi akhir tetap memerlukan penilaian
              ...
              <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-5 gap-2 mb-3.5">
                  {TRAIT_ORDER.map((t) => {
                    ...
                    return (
                      <div key={t} className="rounded-md px-3 py-2.5 border border-slate-200 bg-slate-50">
                        <div className="text-[11px] font-bold text-slate-700">{meta.nameID}</div>
                        <div className="text-[10px] text-slate-400 mb-1">({t})</div>
                        <div className="font-serif text-xl font-bold leading-none text-slate-800">
                          {pct}
                          <span className="text-[10px] font-normal text-slate-400 ml-0.5">/100</span>
                        </div>
                        <div className="bg-slate-200 rounded-full h-1 overflow-hidden mt-1.5">
                          <div className="h-1 rounded-full bg-teal-600" style={{ width: `${pct}%` }} />
                        </div>
                        <span className="text-[10px] font-semibold text-slate-500 mt-1 block">{lvl}</span>
                      </div>
                    );
                  })}
                </div>
                ...
                <div className="bg-slate-50 border-l-4 border-slate-300 rounded-r px-3.5 py-2.5 text-xs text-slate-600 mb-2">
                  ...
                  {TRAIT_ORDER.map((t) => {
                    ...
                    return (
                      <span key={t} className="block mt-2.5">
                        <strong className="text-slate-800">{meta.nameID}:</strong> {txt}
                      </span>
                    );
                  })}
                </div>
              ...
              <>
                ...
                {disc.dominant && DISC_PROFILES[disc.dominant] && (
                  <div className="bg-slate-50 border-l-4 border-slate-300 rounded-r px-3.5 py-2.5 text-xs text-slate-600 leading-relaxed">
                    <strong className="text-slate-800">{DISC_PROFILES[disc.dominant].name}:</strong> {DISC_PROFILES[disc.dominant].short}
                  </div>
                )}
              </>
            ) : (
            ...
            {hol ? (
              <div className="flex items-start gap-5 flex-wrap mb-3.5">
                <div>
                  ...
                  {hol.comboInfo && (
                    <div className="mt-3 px-3 py-2 rounded-lg bg-slate-50 border border-slate-200 text-xs text-slate-600 max-w-[400px]">
                      <div className="font-semibold text-slate-800 mb-0.5">{hol.comboInfo.title}</div>
              ...
              <div key={id} className="mb-3">
                <div className="text-[11px] font-bold uppercase tracking-wider text-slate-600 mb-1.5">
                  ...
                  {ai.isSynthGenerating() && (state['edit_' + id] || '').length > 0 && (
                    <Loader2 className="inline-block h-3 w-3 animate-spin ml-1.5 text-slate-400" />
                  )}
                </div>
                <Textarea
