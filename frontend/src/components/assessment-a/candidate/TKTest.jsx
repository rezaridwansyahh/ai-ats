        ...
        {Object.entries(q.o).map(([k, txt]) => {
          ...
          return (
            <button
              ...
              className={[
                'flex items-start gap-3 px-3.5 py-2.5 rounded-lg border-[1.5px] text-left transition',
                sel ? 'border-pink-600 bg-pink-100/70' : 'border-slate-200 bg-slate-50 hover:border-pink-400 hover:bg-pink-50/40',
              ].join(' ')}
            >
              <div
                className={[
                  'w-6 h-6 rounded grid place-items-center text-[11px] font-bold flex-shrink-0',
                  sel ? 'bg-pink-600 text-white' : 'bg-white border-[1.5px] border-slate-200 text-slate-600',
                ].join(' ')}
              >
                {k}
              </div>
              <div className="text-sm leading-relaxed font-mono">{txt}</div>
            </button>
          );
        })}
      </div>
